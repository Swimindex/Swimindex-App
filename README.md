...
The Worker entry is generated by `npm run build` into `.svelte-kit/cloudflare/_worker.js`.  
Workers Builds must run that **before** Wrangler upload.
...
If the Build command is left empty and deploy is only `npx wrangler versions upload`, the upload fails with missing `_worker.js`.

Shortcut (build + upload in one step): set the deploy / non-production command to `npm run cf:upload`.

For the client demo, add a build variable `PUBLIC_DEMO_MODE=true` (and leave Supabase keys empty).
